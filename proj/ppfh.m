function [uniquePoints, globalstddev,globalmeanhist, distances] = ppfh(ptCloud,radius, cmptype)
...
for n = 1:length(indices)
    ...
    hist = compute_featuresgpu(indices(n), ptCloud, ptCloudgpu, normalsgpu, radius);
    hist = gather(hist);
   ...
   localpointhist = get_mean_hist(hist, "local");
   localhistmap.put(point, localpointhist);

   %disp(localhistmap.keySet());
...
%figure;
avg = gather(avg);
globalmeanhist = get_mean_hist(avg, "global");
